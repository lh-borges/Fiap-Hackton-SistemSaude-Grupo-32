...
public interface ParecerMedicoJpaRepository
    ...
    @org.springframework.data.jpa.repository.Query("select distinct p.resultadoExameId from ParecerMedicoEntity p where p.resultadoExameId in :ids")
    java.util.Set<UUID> resultadosComParecer(@org.springframework.data.repository.query.Param("ids") java.util.Set<UUID> ids);
